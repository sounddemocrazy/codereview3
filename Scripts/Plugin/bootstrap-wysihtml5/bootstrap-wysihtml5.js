!function($, wysi) {
    ...
    var tpl = {
		...
		"avImage": function(locale, options)
		{
			var size = (options && options.size) ? ' btn-'+options.size : '';
            return "<li>" +
              "<div class='bootstrap-wysihtml5-insert-image-av-modal modal fade'>" +
              ($.fn.modalmanager ? "" : "<div class='modal-dialog'>") +
              " <div class='modal-content'>" +
                "<div class='modal-header'>" +
                  "<a class='close' data-dismiss='modal'>&times;</a>" +
                  "<h3>" + locale.avImage.insert + "</h3>" +
                "</div>" +
                "<div class='modal-body'>" +
				"<input type='file' id='wysihtml5ImageUpload' />"+
                  //"<div class='btn btn-success' ngf-select ng-model='ArticleBannerImage' name='ArticleBannerImage' ngf-pattern='image/*' ngf-accept='image/*' ngf-max-size='20MB' ngf-min-height='100'>Select Image</div><img ngf-thumbnail='ArticleBannerImage'>"+
                "</div>" +
                "<div class='modal-footer'>" +
                  "<a href='#' class='btn default' data-dismiss='modal'>" + locale.avImage.cancel + "</a>" +
                  "<a href='#' class='btn btn-primary' data-dismiss='modal'>" + locale.avImage.insert + "</a>" +
                "</div>" +
              "</div>" +
               ($.fn.modalmanager ? "" : "</div>") +
              "</div>" +
              "<a class='btn default" + size + "' data-wysihtml5-command='inserImageAv' title='" + locale.avImage.insert + "' tabindex='-1'><i class='fa fa-picture-o'></i></a>" +
            "</li>";
		}
    };
    ...
    Wysihtml5.prototype = {
        ...
        createToolbar: function(el, options) {
            ...
            for(var key in defaultOptions) {
                ...
                if(value === true) {
					...
					if(key === "avImage")
					{
						this.initInsertImageAv(toolbar);
					}
                }
            }
        ...
        initInsertLink: function(toolbar) {
            ...
            var insertLink = function() {
                ...
                self.editor.currentView.element.focus();
                if (caretBookmark) {
					console.log('insertLink'+caretBookmark);
                  self.editor.composer.selection.setBookmark(caretBookmark);
                  caretBookmark = null;
                }
		...
		//Created by Avinash on 4th May 2017 -- under development
		initInsertImageAv: function(toolbar) {
            var self = this;
            var insertImageAvModal = toolbar.find('.bootstrap-wysihtml5-insert-image-av-modal');
			var insertButton = insertImageAvModal.find('a.btn-primary');
            var caretBookmark;
			
			var uploadImage = function()
			{		 
				var fileInput = $('#wysihtml5ImageUpload').get(0).files[0];
				
				var frmData = new FormData();
				frmData.append("file", fileInput);
				frmData.append("ObjectId", 0);
				frmData.append("RefObjectId", 0);
				frmData.append("PhotoType", 7001);
				...
				$.ajax({				
                    ...
                    console.log(response);
					 console.log(response.Result);
						...
						var imgTag = '<img src="'+ response.Result.UrlOriginal +'" width="500" />';
						self.editor.composer.selection.insertHTML(imgTag);					 
					 },
            ...
            // urlInput.keypress(function(e) {
                // if(e.which == 13) {
                    // uploadImage();
                    // insertImageAvModal.modal('hide');
                // }
            // });

            insertButton.click(uploadImage);

            insertImageAvModal.on('shown', function() {
                urlInput.focus();
            });

            insertImageAvModal.on('hide', function() {
                self.editor.currentView.element.focus();
            });

            toolbar.find('a[data-wysihtml5-command=inserImageAv]').click(function() {
                var activeButton = $(this).hasClass("wysihtml5-command-active");
				console.log('I am here - this is pop up');
                if (!activeButton) {
                    self.editor.currentView.element.focus(false);
                    caretBookmark = self.editor.composer.selection.getBookmark();
                    insertImageAvModal.appendTo('body').modal('show');
					//Edited by Avinash
                   //App.initUniform(); //initialize uniform checkboxes
                    insertImageAvModal.on('click.dismiss.modal', '[data-dismiss="modal"]', function(e) {
                        e.stopPropagation();
                    });
                    return false;
                }
    ...
    var defaultOptions = $.fn.wysihtml5.defaultOptions = {
        ...
        "emphasis": true,
        "lists": true,
        "html": false,//Edited by Avinash
        "link": true,
        "image": false,//Edited by Avinash
		"avImage" : true,
        events: {},
        parserRules: {
            classes: {
